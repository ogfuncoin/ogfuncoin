...
const char * const OGFUNCOIN_CONF_FILENAME = "ogfuncoin.conf";
const char * const OGFUNCOIN_PID_FILENAME = "ogfuncoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ogfuncoind -foo=bar
...
#else
    const char* pszModule = "ogfuncoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ogfuncoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ogfuncoin
    // Mac: ~/Library/Application Support/ogfuncoin
    // Unix: ~/.ogfuncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ogfuncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ogfuncoin";
#else
    // Unix
    return pathRet / ".ogfuncoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", OGFUNCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", OGFUNCOIN_PID_FILENAME)));
}
