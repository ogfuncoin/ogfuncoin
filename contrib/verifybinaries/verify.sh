...
###   ogfuncoincore.org and ogfuncoin.org and compares them.
...
WORKINGDIR="/tmp/ogfuncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ogfuncoincore.org"
HOST2="https://ogfuncoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="ogfuncoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ogfuncoin-',
...
if [ $? -ne 0 ]; then
   echo "ogfuncoin.org failed to provide signature file, but ogfuncoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ogfuncoin.org and ogfuncoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the ogfuncoin Core binary release signing key installed?"
   fi
